...
$date=`/bin/date "+%Y%m%d"`;
chomp($date);
$log="/in/AEM/EMAIL/email.$date.log";
#$log="/in/PEM/u/NetCmmnd/EMAIL/email.test.log";
$condlog="/in/AEM/EMAIL/condition.$date.log";
open LOG, "+>>$log";
...
foreach $ROW (@ROWS)
{
 ...
 if(index($paramName,"EFT STORE")>-1 && $severity ne "10"){
	...
	if($count < 10){
		...
		print LOG localtime(time).`echo "OR=$origin,ET=$eventType,DC=$domainClass,D=$domain,OC=$objectClass,O=$object,PN=$paramName,PV=$paramValue,S=10" | /in/PEM/u/NetCmmnd/EMAIL/emailToSNMP.pl`;
	}
 }
...
sub ELert{
	...
	foreach $ROW (@ROWS)
	{
		...
		#	chomp($ROW);
		#	#print elert $ROW;
		#	print LOG localtime(time)." - ELert touched file for $ROW ".`touch /in/PEM/u/NetCmmnd/EMAIL/ELerts/$ROW`."\n";
		#	last;
		#}elsif(index($ROW, "icket:")>-1){
		#	$started++;
		#	#print elert $ROW;
		#	next;
		#}
